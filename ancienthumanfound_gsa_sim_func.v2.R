...
library(dismo)
library(gbm)
brt.fit <- gbm.step(dat.nona, gbm.x = attr(dat.nona, "names")[1:11], gbm.y = attr(dat.nona, "names")[13], family="gaussian", tolerance = 0.0001, learning.rate = 0.01, bag.fraction=0.75, tree.complexity = 2)
summary(brt.fit)
gbm.plot(brt.fit)
